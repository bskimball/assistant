...
- Closed-loop life improvement (ADR-023): deterministic next-best-action on the dashboard (overdue-aware), personal evening check-in on `DailyPlan`, stable recommendation ids + append-only personal outcomes (`accepted`/`dismissed`/`snoozed`/`completed` + optional helpful flag), weekly focus scheduling records `coach-weekly` outcomes, and coach generation learns from recent outcomes (avoid dismissed/not-helpful, reinforce helpful/completed). Effectiveness rollup lives on `/analytics`.
- Action-first Health workspace (ADR-026): personal, today-only `/health` primes current health records plus light 7-day dashboard context, deterministically selects one next health action, and records append-only `health-next-action` outcomes. Shared client logging helpers power meal estimation, hydration in US fluid ounces, and simple workout writes across Health/Nutrition/Workouts; detailed-route handoffs accept only a validated recommendation id plus allowlisted intent and complete only after the underlying health log succeeds.
