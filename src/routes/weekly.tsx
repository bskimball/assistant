...
import { todayISO, toISODate, toISOWeek, type ISODate } from '@/lib/domain'
...
function Weekly() {
  ...
  const completion = stats && stats.tasksTotal > 0 ? Math.round((stats.tasksCompleted / stats.tasksTotal) * 100) : 0
    ...
    <div className="min-h-dvh bg-background px-4 pb-16 pt-6">
      <div className="mx-auto w-full max-w-[820px]">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <div className="text-xs uppercase tracking-[2px] text-muted-foreground">Weekly Review</div>
          ...
          <div className="flex items-center gap-1.5">
            <Button variant="outline" size="icon" className="size-8" onClick={() => shiftWeek(-1)} aria-label="Previous week">
              <ChevronLeft className="size-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => setAnchor(todayISO())}>This week</Button>
            <Button variant="outline" size="icon" className="size-8" onClick={() => shiftWeek(1)} aria-label="Next week">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatTile icon={Target} label="Task completion" value={`${completion}%`} sub={stats ? `${stats.tasksCompleted}/${stats.tasksTotal}` : '—'} />
          <StatTile icon={Dumbbell} label="Workouts" value={stats ? String(stats.workouts) : '—'} sub="sessions" />
          <StatTile icon={Utensils} label="Avg protein" value={stats ? `${stats.avgProteinPct}%` : '—'} sub="of target" />
          <StatTile icon={Wallet} label="Net worth" value={stats ? `$${stats.netWorth.toLocaleString()}` : '—'} sub={stats ? `${stats.activeDays}/7 active` : ''} />
        </div>
            ...
            {loading ? (
              ...
              <div className="flex items-end justify-between gap-2 h-32">
                {stats?.perDayCompletion.map((d) => {
                  const dow = new Date(d.date + 'T00:00:00').toLocaleDateString([], { weekday: 'short' })
                  return (
                    <div key={d.date} className="flex flex-1 flex-col items-center gap-1">
                      ...
                      <div className="text-[10px] tabular-nums text-muted-foreground/70">{d.total ? `${d.pct}%` : '–'}</div>
                    </div>
                  )
                })}
              </div>
        ...
        <Card className="mb-4">
          <CardHeader>
            <CardTitle className="text-base flex items-center justify-between">
              <span className="flex items-center gap-2"><Sparkles className="size-4 text-primary" /> Coach’s Weekly Review</span>
              <Button variant="ghost" size="sm" className="h-7 gap-1.5 text-xs font-normal" onClick={generate} disabled={narrativeLoading || !stats}>
          ...
          <CardContent className="space-y-4 text-sm">
            ...
            <ReviewField icon={Trophy} label="Wins" value={wins} onChange={setWins} placeholder="One win per line…" />
            <ReviewField icon={TriangleAlert} label="Blockers" value={blockers} onChange={setBlockers} placeholder="What got in the way…" />
            <ReviewField icon={ArrowRight} label="Next week focus" value={nextWeekFocus} onChange={setNextWeekFocus} placeholder="Priorities for next week…" />

            <div>
              <div className="mb-1 text-xs font-medium text-muted-foreground">Reflection</div>
              <Textarea value={reflection} onChange={(e) => setReflection(e.target.value)} rows={3} placeholder="Freeform reflection…" />
            </div>
...
function StatTile({ icon: Icon, label, value, sub }: { icon: typeof Target; label: string; value: string; sub?: string }) {
  return (
