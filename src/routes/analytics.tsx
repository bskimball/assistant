...
import { loadDailyDashboard, loadWorkoutSessions } from '@/lib/server/domain'
...
function Analytics() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [dashboards, sessions] = await Promise.all([
      ...
      const pts: DayPoint[] = dashboards.map((dash, i) => {
        ...
        return {
          ...
          proteinPct: protein > 0 ? Math.min(100, Math.round((protein / Math.max(1, target)) * 100)) : 0,
          waterMl: dash.nutrition?.waterMl ?? 0,
          netWorth: dash.finance?.netWorth ?? 0,
          workouts: allSessions.filter((s) => s.performedAt >= dayStart && s.performedAt <= dayEnd).length,
        }
      })
      setPoints(pts)
    } catch (e) {
    ...
    <div className="min-h-dvh bg-background px-4 pb-16 pt-6">
      <div className="mx-auto w-full max-w-[860px]">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <SummaryTile icon={Target} label="Avg task completion" value={`${avg((p) => p.completionPct)}%`} />
          <SummaryTile icon={Dumbbell} label="Workouts" value={String(totalWorkouts)} sub={`in ${range} days`} />
          <SummaryTile icon={Utensils} label="Avg protein" value={`${avg((p) => p.proteinPct, true)}%`} sub="of target" />
          <SummaryTile
            icon={Wallet}
            label="Net worth"
        ...
        {loading ? (
          <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">Crunching {range} days…</CardContent></Card>
        ) : points.length === 0 ? (
          <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">No data yet. Log a few days from the dashboard.</CardContent></Card>
        ) : (
          <div className="space-y-4">
            ...
            {latestNetWorth > 0 && (
              <ChartCard icon={Wallet} title="Net worth ($)">
                <LineChart points={points.filter((p) => p.netWorth > 0)} sel={(p) => p.netWorth} unit="$" prefix />
              </ChartCard>
            )}
          </div>
        )}
    ...
    <div className="rounded-xl border bg-card p-3">
      ...
      <div className="mt-1 flex items-center gap-1.5 text-xl font-semibold tabular-nums">
        ...
        {trend !== undefined && trend !== 0 && (
          <span className={`flex items-center text-xs ${trend > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
...
function BarsChart({ points, sel, unit = '' }: { points: DayPoint[]; sel: (p: DayPoint) => number; unit?: string }) {
  const vals = points.map(sel)
      ...
      <div className="flex items-end justify-between gap-0.5" style={{ height: H }}>
        {points.map((p, i) => (
          <div
            ...
            style={{ height: `${Math.max(2, (vals[i] / top) * 100)}%`, opacity: vals[i] ? 1 : 0.15 }}
