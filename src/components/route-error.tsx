...
import { WarningIcon, SquaresFourIcon, ArrowsClockwiseIcon } from "@phosphor-icons/react";
    ...
    <div className="flex min-h-[50dvh] items-center justify-center px-4 py-12 sm:px-6">
      <div className="mx-auto w-full max-w-md text-center">
        <div className="mx-auto mb-5 flex size-14 items-center justify-center rounded-2xl border border-destructive/20 bg-destructive/10">
          <WarningIcon weight="duotone" className="size-7 text-destructive" />
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
          <Button size="sm" className="gap-1.5" onClick={reset}>
            <ArrowsClockwiseIcon weight="duotone" className="size-4" /> Try again
          </Button>
          <Button asChild variant="outline" size="sm" className="gap-1.5">
            <Link to="/">
              <SquaresFourIcon weight="duotone" className="size-4" /> Back to dashboard
            </Link>
          </Button>
        </div>
      </div>
